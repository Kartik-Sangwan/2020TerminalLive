...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def attack(self, game_state):
        # CHECK which way to attach
        # add emps
        # if game_state.get_resource(BITS) >= 10:
        #     game_state.attempt_remove([14,10])
        if game_state.get_resource(BITS) >= 14:
            emp_or_ping = random.randint(0,1)
            if emp_or_ping == 0:
                game_state.attempt_spawn(EMP, [14,0], math.floor(game_state.get_resource(BITS)))
            else:            
                game_state.attempt_spawn(PING, [13, 0], math.floor(game_state.get_resource(BITS)))
    def build_defences(self, game_state):
        ...
        pink_destructors_points = [[1, 12], [26, 12]]
        pink_filters_points = [[0, 13], [1, 13], [2, 13], [25, 13], [26, 13], [27, 13], [2, 12], [25, 12], [2, 11], [25, 11]]
        blue_filters_points = [[14,10], [3, 12], [4, 12], [5, 12], [6, 12], [7, 12], [8, 12], [9, 12], [10, 12], [11, 12], [12, 12], [13, 12], [15, 12], [16, 12], [17, 12], [18, 12], [19, 12], [20, 12], [21, 12], [22, 12], [23, 12], [24, 12], [12, 11], [16, 11], [13, 10], [15, 10]]
        teal_destructors_points = [[7, 11], [11, 11], [15, 11], [21, 11]]
        yellow_destructors_points = [[4, 11], [5, 11], [9, 11], [13, 11], [17, 11], [18, 11], [20, 11], [23, 11], [24, 11]]
        orange_encryptors_points = [[13, 3], [14, 3], [13, 2], [14, 2]]
        ...
        # for item in blue_filters_points:
        #     if item == [14,10]:
        #         if game_state.get_resource(BITS) < 12:
        #             game_state.attempt_spawn(FILTER, [14,10])
        #     else:
        #         game_state.attempt_spawn(FILTER, item)

        game_state.attempt_spawn(ENCRYPTOR, orange_encryptors_points)
        game_state.attempt_spawn(DESTRUCTOR, yellow_destructors_points)

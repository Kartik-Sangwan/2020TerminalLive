...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def attack(self, game_state):
        ...
        num = math.ceil(game_state.get_resource(MP)/2)

        if game_state.turn_number <= 5:
            game_state.attempt_spawn(INTERCEPTOR, [14, 0], num)
            game_state.attempt_spawn(INTERCEPTOR, [13, 0], num + 1)
        else:
            if game_state.turn_number % 2 == 0:
                game_state.attempt_remove(self.l)
                self.r_build = True
                self.l_build = False
                game_state.attempt_spawn(SCOUT, [13, 0], 100)
            else:
                game_state.attempt_remove(self.r)
                self.l_build = True
                self.r_build = False
    ...
    def build_defences(self, game_state):
        ...
        blue_filters_points = [[3, 13], [4, 13], [23, 13], [24, 13]]

        blue_encryptors_points.reverse()
        if not self.r_build:
            game_state.attempt_spawn(WALL, self.l)
        elif not self.l_build:
            game_state.attempt_spawn(WALL, self.r)

        if game_state.turn_number >= 2:
            game_state.attempt_spawn(WALL, blue_filters_points)
        if game_state.turn_number == 5:
            game_state.attempt_upgrade(blue_filters_points)
        game_state.attempt_spawn(FACTORY, blue_encryptors_points[:4], 4)
        game_state.attempt_upgrade([[14, 2]])
        game_state.attempt_spawn(
            TURRET, pink_destructors_points)
